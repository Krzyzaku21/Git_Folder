...
# TODO - check_collection - sprawdzanie kolekcji i dodawanie jej
    ...
    def check_collection(cls):
        ...
        if (cls.COLLECTION_NAME in collection_list) is True:
            print(
                f"This collection {cls.COLLECTION_NAME} exists in {collection_list}")
            cls.COLLECTION = cls.DATABASE[cls.COLLECTION_NAME]
        else:
            print(
                f"This: {cls.COLLECTION_NAME} don't exist in datebase {collection_list}")
            as_chose = str(
                input(r"Write 'y' if you want create collection or write 'n' if you don't"))
            if as_chose == "y":
                cls.DATABASE.create_collection(cls.COLLECTION_NAME)
                print(
                    f"Created new collection: {cls.COLLECTION_NAME} in datebase {cls.CLIENT.list_database_names()}")
                collection_list = cls.DATABASE.list_collection_names()
                print(collection_list)
                cls.COLLECTION = cls.DATABASE[cls.COLLECTION_NAME]
            else:
